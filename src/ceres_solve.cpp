...
public:
                     ...
                     T* residual) const
    {
        ...
        SE3_j = Sophus::SE3Group<T>(quat.toRotationMatrix(),translation);
        // ===============================
        ...
        // SE(3)_i^j = ( SE(3)_j^0 )^-1 * SE(3)_i^0
        Sophus::SE3Group<T> SE3_i_2_j = SE3_j.inverse() * key_frame_SE3;
...
void ceres_solve(int head)
{
    ...
    Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic,Eigen::RowMajor> rowmajor_img[4];
    ceres::Grid2D<double,1> *array[4];
    ceres::BiCubicInterpolator< ceres::Grid2D<double,1> > *interpolator[4];

    for (int i=0;i<4;i++)
    {
        rowmajor_img[i] = graph.state[head+i].img_data[calc_level];  // current frame
        array[i] = new ceres::Grid2D<double,1>(rowmajor_img[i].data(),0,rowmajor_img[i].rows(),0,rowmajor_img[i].cols());
        interpolator[i] = new ceres::BiCubicInterpolator< ceres::Grid2D<double,1> >(*array[i]);
        ...
        for (int v=1;v<graph.state[key_frame_no].img_data[calc_level].rows()-1;v++)
            ...
            {
                cost_function = new ceres::AutoDiffCostFunction<dvo_functor, 1, 3,4, 7>
                                    (
                                     new dvo_functor(
                                                     graph.state[key_frame_no].img_data[calc_level],graph.state[key_frame_no].depth[calc_level],*interpolator[i],
                ...
                problem.AddResidualBlock(cost_function,NULL, &p[i][0],&q[i][0],key_frame_RT);
            }
    }
